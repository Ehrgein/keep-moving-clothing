        ...
        <div className={cart.cartSlider ?
            ...
            {cart.items.length > 0 ?
             <div className='mt-10 mx-4 cart-anim overflow-y-scroll mobilexs:text-sm'>
                        {cart.items.map(item =>
                        <div key={item.id} className='scrollcart cart-anim flex justify-start cartborder my-2' >
                            ...
                            <div className='uppercase font-medium ml-2 my-6 tracking-wide
                             mobileL:w-[65%] mobilexs:w-[80%]'>
                                ...
                                <div className='flex items-center h-10 gap-6 '>
                                    <AiOutlineMinus className='4k:w-[15px] 4k:h-[15px]' onClick={() => cart.removeOneFromCart(item.id)}/>
                                    <p className='4k:text-lg text-base'>{cart.getProductQuantity(item.id)}</p>
                                    <AiOutlinePlus className='4k:w-[15px] 4k:h-[15px]' onClick={ () =>  cart.addOneToCart(item.id)}/>
                ...
                {onpurchasemodal && usercontext.user ? <CheckoutModal onpurchasemodal={onpurchasemodal} setonPurchasemodal={setonPurchasemodal} user={usercontext.user}/> 
                : null}
                {onerrormodal ? <ErrorModal onerrormodal={onerrormodal} setonErrorModal={setonErrorModal} handleCart={() => cart.handleCart()}/>
                :
                null}
            </div>
