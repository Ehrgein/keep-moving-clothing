    ...
    <div>      
        ...
        <motion.div ref={carousel} className="carousel">
          ...
          dragConstraints={{right:0, left:-width}} className='inner-carousel'>
            {productscontext.slice(-10).map(item => 
            <motion.div className='flex flex-col uppercase justify-center desktop:w-[280px] laptopL:w-[160px] md:w-[150px] mt-11 text-sm laptopL:mx-12 md:mx-20 lg:mx-12' 
            key={item.id} categories={item.categories} itemname={item.name} itemprice={item.price} itemimg={item.prod_img} >
              ...
              <motion.div className='item'>
                <Link to={`/products/${item.categories}/${item.id}`}>
                  <img onMouseDown={e => e.preventDefault()} className='grabimg' src={item.prod_img}/>
                </Link>
              </motion.div>
