  ...
  return (
    <div>      
          ...
          dragConstraints={{right:0, left:-width}} className='inner-carousel ml-6'>
            ...
            key={item.id} categories={item.categories} itemname={item.name} itemprice={item.price} itemimg={item.prod_img} >
              ...
              <div className='4k:mt-4 desktop:mt-4 laptopL:mt-4 tablet:mt-2 '>
                ...
                <Link>
                  <p className='text-center 4k:text-lg text-sm mt-2'> {item.brand} </p>
                </Link>
                <p className='text-base mt-1 font-semibold text-center 4k:text-lg desktop:text-base'> $ {item.price}  </p>      
              </div>
            </motion.div>
            )}
          </motion.div>
        ...
        <div className='flex justify-center my-12'>
            <Link className='text-white font-bold text-xl px-7 py-2 bg-black' to={`/newarrivals`} >
              VIEW ALL
            </Link>
        </div>
