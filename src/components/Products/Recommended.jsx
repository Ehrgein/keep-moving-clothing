  ...
  return (
    <div>
      ...
      <div className="hidden md:flex justify-center flex-wrap">
        {productscontext
          ...
          .map((item) => (
            <div
              key={item.id}
              className="flex uppercase justify-center items-center 4k:w-[350px] desktop:w-[280px] laptopL:w-[240px] laptop:w-[170px] md:w-[150px] my-10 text-sm
              ...
              <ul className="text-sm text-center">
                ...
                >
                  <img
                    ...
                    className="h-auto text-center md:mb-0 mb-4"
                    src={item.prod_img}
                  />
                </Link>
                <li className="flex justify-center items-center font-bold text-center 4k:text-xl desktop:text-base  md:text-sm h-10">
                  {" "}
                  {item.name}{" "}
                </li>
                <li className="text-center 4k:text-xl desktop:text-base md:text-sm mt-1">
                  {" "}
                  {item.brand}
                </li>
                <p className="mt-1 font-semibold text-center 4k:text-xl desktop:text-base">
                  $ {item.price}
                </p>
              </ul>
            </div>
      ...
      <div className="hidden md:flex justify-center mb-12">
        ...
        >
          <div className="flex border-2 4k:text-2xl border-black text-black bg-white font-bold text-medium px-5 py-4 uppercase items-center">
            ...
            .map((item) => (
              <div key={item.id} className="mx-auto my-auto">
                <Link to={`/products/${item.categories}/${item.id}`}>
                  <img
                    className="tablet:w-[140px] tablet:h-[140px] mobilexs:w-[180px] mobilexs:h-[170px] mt-4"
                    src={item.prod_img}
                    alt={`${item.name} ${item.brand} $${item.price}`}
                  />
                </Link>
                ...
                <p className="text-center text-base font-semibold mt-1 mr-1">
                  {" "}
                  $ {item.price}
                </p>
              </div>
