...
function ProductsByBrand() {
    ...
    const sortedItemsSelect = (e) => {
      ...
      if(value === 'ascending' ){
        const ascendingOrder = [...productscontext].filter(item => item.brand === brand).sort((a, b) => a.price - b.price)
        setSortedItems(ascendingOrder)

      }
      if(value === 'descending'){
        const descendingOrder = [...productscontext].filter(item => item.brand === brand).sort((a, b) => b.price - a.price)
        setSortedItems(descendingOrder)
        
      }
      if(value === 'az' ){
        const ascendingaz = [...productscontext].filter(item => item.brand === brand).sort((a, b) =>a.name > b.name ? 1 : -1,);
        setSortedItems(ascendingaz)

      }
      if(value === 'za' ){
        const descendingza = [...productscontext].filter(item => item.brand === brand).sort((a, b) =>a.name > b.name ? -1 : 1,);
