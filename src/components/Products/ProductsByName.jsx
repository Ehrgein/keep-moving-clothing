...
function ProductsByName() {
    ...
    const designerproducts = productscontext.filter((word) => word.name.toUpperCase().includes("Garret".toUpperCase()))
    ...
    const sortedItemsSelect = (e) => {
      ...
      if(value === 'ascending' ){
        const designerproducts = productscontext.filter((word) => word.name.toUpperCase().includes("Garret".toUpperCase()))
        const ascendingOrder = [...designerproducts].sort((a, b) => a.price - b.price)
        setSortedItems(ascendingOrder)

      }
      if(value === 'descending'){
        const designerproducts = productscontext.filter((word) => word.name.toUpperCase().includes("Garret".toUpperCase()))
        const descendingOrder = [...designerproducts].sort((a, b) => b.price - a.price)
      ...
      if(value === 'az' ){
        ...
        const ascendingaz = [...designerproducts].sort((a, b) =>a.name > b.name ? 1 : -1,);
        setSortedItems(ascendingaz)

      }
      if(value === 'za' ){
        const designerproducts = productscontext.filter((word) => word.name.toUpperCase().includes("Garret".toUpperCase()))
        const descendingza = [...designerproducts].sort((a, b) =>a.name > b.name ? -1 : 1,);
